...
def run_pytest_with_code(cut_code: str, test_code: str, timeout_s=20):
    ...
    try:
        ...
        # 写入测试。与变异路径保持一致：星号导入让测试可以直接调用裸函数名
        # （生成的测试写的是 `decode(...)` 而非 `CUT.decode(...)`）。
        test_path = os.path.join(tmpdir, "test_cut.py")
        if "import CUT" in test_code:
            fixed = test_code.replace("import CUT", "import cut as CUT")
        else:
            fixed = "from cut import *\nimport cut as CUT\n" + test_code
...
def run_mutation_kill_detail(cut_code: str, test_code: str, timeout_s=60, operators=None):
    """
    与 run_mutation_kill 类似，但返回 (killed_ids:set[str], total:int)。
    通过 MutPy 的 --report YAML 输出解析每个变异体的状态，
    供 SM-GRPO 的有状态边际击杀奖励使用。
    解析失败时回退到轻量文本变异（id 前缀 'fb'）。
    """
    tmpdir = tempfile.mkdtemp(prefix="mutpyd_")
    try:
        with open(os.path.join(tmpdir, "cut.py"), "w", encoding="utf-8") as f:
            f.write(cut_code)
        test_code_fixed = test_code if "import CUT" in test_code else "from cut import *\n" + test_code
        ...
        if os.environ.get("TRIU_DISABLE_MUTPY"):
            # 训练期使用可复现的轻量近似变异（论文 3.3 Remark 允许）
            return _fallback_mutation_kill_detail(cut_code, test_code, timeout_s)

        mutpy_cmd = _which_mutpy()
        if mutpy_cmd is None:
            return _fallback_mutation_kill_detail(cut_code, test_code, timeout_s)
        ...
        try:
            ...
            try:
                p.communicate(timeout=timeout_s + 30)
            except subprocess.TimeoutExpired:
                _kill_process_tree(p)
                return _fallback_mutation_kill_detail(cut_code, test_code, timeout_s)
        except OSError:
        ...
        if os.path.exists(report_path):
            try:
                ...
                with open(report_path, "r", encoding="utf-8") as f:
                    _txt = f.read()
                # 去掉 !!python 对象标签后再 safe_load
                rep = yaml.safe_load(re.sub(r"!!python\S*", "", _txt))
                killed, total = set(), 0
                for m in (rep or {}).get("mutations", []):
                    ...
                    mid = f"{sub[0].get('operator','?')}_{sub[0].get('lineno','?')}_{m.get('number', total)}"
...
def _fallback_mutation_kill_detail(cut_code: str, test_code: str, timeout_s=40):
    """文本级回退：返回 (killed_ids, total)，id 形如 'fb0'（确定性顺序，可跨调用对齐）。"""
    mutants = []
    pairs = [
        ("+","-"),("-","+"),("*","//"),("//","*"),
        ("==","!="),("!=","=="),(">","<"),("<",">")
    ...
    for i in range(total):
        r = run_pytest_with_code(mutants[i], test_code, timeout_s=timeout_s)
        if not r["ok"]:
            killed.add(f"fb{i}")
    return killed, total
