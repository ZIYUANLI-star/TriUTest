...
def main():
    ...
    # 断点续跑：若 output_dir 已有 step_N checkpoint，用最新 checkpoint 作为
    # policy 起点（ref 仍为原配置），并从对应步数继续
    start_step = 0
    import glob as _glob
    _ckpts = []
    for _d in _glob.glob(os.path.join(out_dir, "step_*")):
        _m = re.match(r".*step_(\d+)$", _d)
        if _m and os.path.exists(os.path.join(_d, "adapter_config.json")):
            _ckpts.append((int(_m.group(1)), _d))
    if _ckpts:
        _ckpts.sort()
        start_step, _latest = _ckpts[-1]
        cfg["policy_model"] = _latest
        print(f"[GRPO] resuming policy from {_latest} (step {start_step})", flush=True)
    ...
    # 规约锚定扰动（spec-anchored prompt perturbation）：
    # 以概率 p_perturb 在提示中展示"被注入单点变异的 CUT"，而奖励（基线门、
    # 变异杀伤）仍针对原始正确 CUT 计算。照抄所示实现的候选会在基线门被罚，
    # 只有依据函数描述（规约）写断言的候选获得奖励——把"规约忠实性"变成
    # 可优化信号，抑制 RL 的实现顺应偏置（该偏置会压低 FTR）。
    p_perturb = float(cfg.get("reward", {}).get("p_perturb", 0.0))
    _pert_rnd = random.Random(cfg.get("seed", 42))
    _PAIRS = [("+", "-"), ("-", "+"), ("*", "//"), ("//", "*"),
              ("==", "!="), ("!=", "=="), (">", "<"), ("<", ">"),
              (">=", "<"), ("<=", ">")]

    def perturb_code(code: str, rnd: random.Random) -> str:
        cands = []
        for a, b in _PAIRS:
            idx = code.find(a)
            if idx != -1:
                cands.append((idx, a, b))
        if not cands:
            return ""
        idx, a, b = rnd.choice(cands)
    ...
    for ex in arr:
        ...
        if cut and p_perturb > 0 and _pert_rnd.random() < p_perturb:
            mut = perturb_code(cut, _pert_rnd)
            if mut and mut != cut and cut in p:
                p = p.replace(cut, mut, 1)
                n_pert += 1
        # cut_code 保持为原始正确实现：奖励一律对它计算
        batch_data.append({"prompt": p, "cut_code": cut})
    if p_perturb > 0:
        print(f"[GRPO] spec-anchored perturbation: {n_pert}/{len(batch_data)} "
              f"prompts show a mutated CUT (p={p_perturb})", flush=True)
    ...
    stateful_reward = None
    if cfg["reward"].get("stateful", False):
        # SM-GRPO：有状态边际击杀奖励（论文 3.3 节）
        from src.reward.stateful import StatefulMutantReward
        stateful_reward = StatefulMutantReward(
            weights=cfg["reward"]["weights"],
            time_budget_s=cfg["reward"]["time_budget_s"],
            operators=cfg["reward"].get("operators", None),
        )
        reward_fn = stateful_reward
    else:
    ...
    for step in range(start_step, steps):
        ...
        # SM-GRPO：用被选中的最优候选更新变异体覆盖状态 S[u]
        if stateful_reward is not None:
            for bp, ch in zip(batch_prompts, chosen):
                try:
                    stateful_reward.update_state(bp["cut_code"], ch)
                except Exception:
                    pass
